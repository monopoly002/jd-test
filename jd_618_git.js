		...
		if (secretp != "") {
			...
			for (var i = 0; i < itemsList.length; i++) {
				...
				if (item.taskId == 101) {
					...
					//await wait(1000);
					await wait(Math.floor(Math.random() * 8000) + 1000)
				...
				for (var i = 0; i < addProductVos.length; i++) {
					...
					if (item.status == 1) {
						...
						for (let j = item.times; j < item.maxTimes && j < productInfoVos.length; j++) {
							...
							}, Math.floor(Math.random() * 9000) + 1000 * 1 * count);
							count++;
						}
					}
				}
				...
				for (var i = 0; i < viewProductVos.length; i++) {
					...
					if (item.status == 1) {
						...
						for (let j = item.times; j < item.maxTimes && j < productInfoVos.length; j++) {
							...
							}, Math.floor(Math.random() * 8000) + 1000 * 1 * count);
							count++;
						}
					}
				}
					...
					if (data.success) {
						if (data.result) {
							if (data.result.taskToken) {
								...
								}, Math.floor(Math.random() * 10000) + 1000 * 10);
							} else {
